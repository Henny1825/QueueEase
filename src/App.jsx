...
import NextInQueue from "./pages/NextInQueue"
import ItsYourTurn from "./pages/ItsYourTurn"
...
const GlobalStyle = () => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    :root {
      --ink:#0d0d0d; --paper:#f5f2ec; --cream:#ede9e0;
      --teal:#00897b; --teal2:#00bfa5; --amber:#ffb300;
      --rose:#e53935; --muted:#6b6860; --border:#d6d1c7;
      --card:#ffffff; --radius:12px; --shadow:0 2px 16px rgba(0,0,0,0.08);
    }
    body { font-family:'DM Sans',sans-serif; background:var(--paper); color:var(--ink); }
    .syne { font-family:'Syne',sans-serif; }
    .mono { font-family:'DM Mono',monospace; }
    ::-webkit-scrollbar{width:6px} ::-webkit-scrollbar-track{background:var(--cream)} ::-webkit-scrollbar-thumb{background:var(--border);border-radius:3px}
    ...
    .fade-up{animation:fadeUp .45s ease both} .pop{animation:pop .4s cubic-bezier(.34,1.56,.64,1) both}
    .spinning{animation:spin 1s linear infinite} .pulsing{animation:pulse 1.6s ease-in-out infinite}
    .stamp{display:inline-block;border:3px solid var(--teal);border-radius:6px;padding:2px 10px;font-family:'DM Mono',monospace;font-size:11px;letter-spacing:.12em;text-transform:uppercase;color:var(--teal)}
    .stamp.rose{border-color:var(--rose);color:var(--rose)} .stamp.amber{border-color:var(--amber);color:#9a6e00}
    .tab-pill{display:flex;gap:4px;background:var(--cream);border-radius:999px;padding:4px;border:1px solid var(--border)}
    .tab-pill button{border:none;cursor:pointer;border-radius:999px;padding:7px 20px;font-family:'DM Sans',sans-serif;font-size:13px;font-weight:500;background:transparent;color:var(--muted);transition:all .2s}
    .tab-pill button.active{background:var(--teal);color:#fff}
    .card{background:var(--card);border-radius:var(--radius);border:1px solid var(--border);box-shadow:var(--shadow)}
    .btn{display:inline-flex;align-items:center;gap:8px;border:none;cursor:pointer;border-radius:8px;padding:11px 22px;font-family:'DM Sans',sans-serif;font-size:14px;font-weight:500;transition:all .18s}
    ...
    .field input,.field select,.field textarea{border:1px solid var(--border);border-radius:8px;padding:10px 14px;font-family:'DM Sans',sans-serif;font-size:14px;color:var(--ink);background:var(--paper);outline:none;transition:border-color .18s}
    .field input:focus,.field select:focus{border-color:var(--teal)}
    .live-dot{width:8px;height:8px;border-radius:50%;background:var(--teal2);box-shadow:0 0 0 3px rgba(0,191,165,.25);animation:pulse 1.6s ease-in-out infinite}
    ...
    .ticket-card{background:var(--card);border:1px solid var(--border);border-radius:16px;overflow:hidden;position:relative}
    .ticket-card::before{content:'';display:block;height:6px;background:linear-gradient(90deg,var(--teal),var(--teal2),var(--amber))}
    .notch{width:22px;height:22px;border-radius:50%;background:var(--paper);border:1px solid var(--border)}
    .ch-badge{display:inline-flex;align-items:center;gap:5px;padding:3px 10px;border-radius:999px;font-size:11px;font-weight:600;background:var(--cream);color:var(--muted);border:1px solid var(--border)}
    ...
    .hide-sm{display:flex} .show-sm{display:none}
...
export default function QueueEase() {
  ...
  const callNext = (orgId,svcIdx) => {
    setQueue(q=>{
      ...
      if(waitIdx>=0){tickets[waitIdx].status="called";showToast(`Called ${tickets[waitIdx].id} — please proceed to counter.`,"info");}
      return nq;
    });
  };
  ...
  const cancelTicket = (ticketId,orgId,service) => {
    setQueue(q=>{
      ...
      if(svcEntry){const t=svcEntry.tickets.find(t=>t.id===ticketId);if(t)t.status="cancelled";}
      return nq;
    });
  ...
  return (
    ...
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column"}}>
      ...
      {/* HEADER */}
      <header style={{background:"var(--card)",borderBottom:"1px solid var(--border)",padding:"0 24px",display:"flex",alignItems:"center",justifyContent:"space-between",height:60,position:"sticky",top:0,zIndex:100}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <div style={{width:50,height:50,borderRadius:8,background:"linear-gradient(135deg,var(--teal),var(--teal2))",display:"flex",alignItems:"center",justifyContent:"center"}}>
            <img src={Queueease} alt="QueueEase" style={{width:50,height:"auto",borderRadius:8}}/>
          </div>
      ...
      {currentView!=="landing" && currentView!=="ticket" && (
        <div style={{gap:0,overflowX:"auto",background:"var(--card)",borderBottom:"1px solid var(--border)",padding:"0 8px"}} className="show-sm">
          {navItems.map(n=>(
            <button key={n.id} onClick={()=>setView(n.id)} style={{border:"none",cursor:"pointer",borderRadius:0,padding:"10px 14px",fontSize:12,fontWeight:500,background:"transparent",whiteSpace:"nowrap",color:currentView===n.id?"var(--teal)":"var(--muted)",borderBottom:currentView===n.id?"2px solid var(--teal)":"2px solid transparent",fontFamily:"'DM Sans',sans-serif"}}>
              {n.label}
            </button>
          ))}
        </div>
      ...
      {/* TOAST */}
      {toast && (
        <div className="pop" style={{position:"fixed",bottom:24,right:24,zIndex:999,background:toast.type==="warning"?"#fff8e1":toast.type==="info"?"#e3f2fd":"#e8f5e9",border:`1px solid ${toast.type==="warning"?"#ffe082":toast.type==="info"?"#90caf9":"#a5d6a7"}`,borderRadius:10,padding:"12px 18px",boxShadow:"0 4px 20px rgba(0,0,0,.12)",fontSize:13,fontWeight:500,maxWidth:300,color:toast.type==="warning"?"#7c5700":toast.type==="info"?"#0d47a1":"#1b5e20"}}>
          {toast.msg}
        </div>
      )}
      ...
      <main style={{flex:1,padding:"28px 20px",maxWidth:1100,margin:"0 auto",width:"100%"}}>
        ...
        {currentView==="signup" && (
          <ManagerSignupForm onSignupSuccess={()=>setView("login")} onLoginClick={()=>setView("login")}/>
        )}
          ...
          return (
            <QueueConfirmation
              ...
              onConfirm={()=>{
                ...
                setQueue(q=>{
                  const nq={...q};
                  const entry=nq[pendingTicket.orgId]?.find(s=>s.service===pendingTicket.service);
        ...
        {currentView==="ticket" && currentTicket && (()=>{
          const svcEntry=queue[currentTicket.orgId]?.find(s=>s.service===currentTicket.service);
          const waitingTickets=svcEntry?.tickets.filter(t=>t.status==="waiting")||[];
          const calledTicket=svcEntry?.tickets.find(t=>t.status==="called");
          const posIndex=waitingTickets.findIndex(t=>t.id===currentTicket.id);
          const peopleAhead=posIndex>=0?posIndex:0;
          const orgAvg=ORGS.find(o=>o.id===currentTicket.orgId)?.avgMins||8;

          if(currentTicket.status==="done") return (
            <ServiceCompleted
              service={currentTicket.service}
              orgName={currentTicket.orgName}
              servedAt={fmtTime(new Date())}
              completedDate={new Date().toLocaleDateString()}
              totalWaitMins={peopleAhead*orgAvg}
              onJoinAnother={()=>setView("customer")}
              onReturnHome={()=>setView("landing")}
            />
          );

          if(currentTicket.status==="called") return (
            <ItsYourTurn
              ticketId={currentTicket.id}
              orgName={currentTicket.orgName}
              onImHere={()=>showToast("Officer notified you're here.","info")}
              onNeedHelp={()=>showToast("Help request sent.","info")}
            />
          );
          ...
          return (
            <QueueTicket
              ...
              estimatedWait={peopleAhead*orgAvg}
              joinedText={fmtTime(currentTicket.joinedAt)}
              nowServingId={calledTicket?.id||"—"}
              onViewLiveStatus={()=>setView("live_status")}
              onLeaveQueue={()=>{cancelTicket(currentTicket.id,currentTicket.orgId,currentTicket.service);setView("customer");}}
            />
          );
        })()}

        {/* LIVE STATUS */}
        {currentView==="live_status" && currentTicket && (()=>{
          const svcEntry=queue[currentTicket.orgId]?.find(s=>s.service===currentTicket.service);
          const waitingTickets=svcEntry?.tickets.filter(t=>t.status==="waiting")||[];
          const calledTicket=svcEntry?.tickets.find(t=>t.status==="called");
          const posIndex=waitingTickets.findIndex(t=>t.id===currentTicket.id);
        ...
        {currentView==="admin" && (
          <div className="fade-up">
            ...
            <div style={{display:"flex",gap:10,marginBottom:20,flexWrap:"wrap"}}>
              <button className="btn btn-primary" onClick={()=>callNext(aOrg,aSvc)}><Icon name="bell" size={15}/>Call Next</button>
              <button className="btn btn-ghost" onClick={()=>markDone(aOrg,aSvc)}><Icon name="check" size={15}/>Mark Served</button>
              <button className="btn btn-ghost" onClick={()=>setTick(x=>x+1)}><Icon name="refresh" size={15}/>Refresh</button>
            </div>
            <div className="card" style={{overflow:"hidden"}}>
              <div style={{padding:"16px 20px",borderBottom:"1px solid var(--border)",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <span style={{fontWeight:700,fontSize:14}}>{adminOrg?.services[aSvc]} — Queue</span>
                <span className="mono" style={{fontSize:12,color:"var(--muted)"}}>{adminQueue?.tickets.filter(t=>t.status==="waiting").length||0} waiting</span>
              </div>
              <div style={{overflowX:"auto"}}>
                <table className="tbl">
                  <thead><tr><th>#</th><th>Token</th><th>Phone</th><th>Channel</th><th>Joined</th><th>Est. Wait</th><th>Status</th></tr></thead>
                  <tbody>
                    {adminQueue?.tickets.map((t,i)=>{
                      ...
                      return (
                        <tr key={t.id}>
                          ...
                          <td style={{fontSize:12}}>{t.status==="waiting"?`~${(waitPos+1)*adminOrg.avgMins} min`:"—"}</td>
                          <td>
                ...
                {ORGS.map(o=>{
                  ...
                  return (
                    <div key={o.id}>
                      ...
                      <div className="q-bar">
                        <div className="q-bar-fill" style={{width:`${pct}%`,background:pct>70?"linear-gradient(90deg,var(--rose),#ff8a65)":pct>40?"linear-gradient(90deg,var(--amber),#ffca28)":"linear-gradient(90deg,var(--teal),var(--teal2))"}}/>
                      </div>
                      <div style={{display:"flex",gap:12,marginTop:6,flexWrap:"wrap"}}>
                        {o.services.map(svc=>{
                          const sw=queue[o.id]?.find(s=>s.service===svc)?.tickets.filter(t=>t.status==="waiting").length||0;
                          return <span key={svc} style={{fontSize:11,color:"var(--muted)"}}>{svc}: <strong>{sw}</strong></span>;
                        })}
                      </div>
                    </div>
                  );
            ...
            <div className="card" style={{padding:24}}>
              ...
              <div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
                {Object.entries(CHANNELS).map(([k,v])=>{
                  const count=Object.values(queue).flat().flatMap(s=>s.tickets).filter(t=>t.channel===k).length;
