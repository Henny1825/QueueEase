...
/**
 * ManagerServiceDashboard.jsx
 *
 * Manager-facing Service Management screen.
 * Matches Figma: "Service Management" empty state, list state,
 * "Create New Service" form, "Service Details" view, "Edit Service" form,
 * and "Deactivate Service" confirmation modal.
 *
 * This is a content-area component only — it does not render the sidebar
 * or header chrome (those live in the surrounding layout, same as
 * UserView.jsx / DigitalTicket.jsx in this codebase).
 *
 * API WIRING:
 * All network calls are isolated in the `serviceApi` object at the bottom
 * of this file. Replace the bodies of those functions with real calls to
 *   GET    /service
 *   POST   /service
 *   PATCH  /service/{id}
 *   DELETE /service/{id}
 * The component only calls serviceApi.* — no fetch/axios calls are
 * scattered through the JSX, so wiring later means editing one place.
 *
 * No TypeScript, no Redux, matches existing project conventions.
 */

// ---------------------------------------------------------------------
// Mock data (stand-in for GET /service)
// ---------------------------------------------------------------------
...
const MOCK_SERVICES = [
  ...
  { id: "PASSPORT002", name: "Passport Renewal", department: "Immigration", location: "Lagos Office", status: "Active", queueEnabled: true, queuePrefix: "B", dailyCapacity: 100, avgServiceMins: 15, maxQueueSize: 200, openingTime: "08:00", closingTime: "16:00", operatingDays: ["Mon", "Tue", "Wed", "Thu", "Fri"], todaysQueue: 40, completedToday: 30, avgWaitMins: 12, utilization: 60 },
  { id: "PASSPORT003", name: "Passport Renewal", department: "Immigration", location: "Lagos Office", status: "Active", queueEnabled: true, queuePrefix: "C", dailyCapacity: 100, avgServiceMins: 15, maxQueueSize: 200, openingTime: "08:00", closingTime: "16:00", operatingDays: ["Mon", "Tue", "Wed", "Thu", "Fri"], todaysQueue: 20, completedToday: 18, avgWaitMins: 9, utilization: 40 },
  { id: "PASSPORT004", name: "Passport Renewal", department: "Immigration", location: "Lagos Office", status: "Active", queueEnabled: true, queuePrefix: "D", dailyCapacity: 100, avgServiceMins: 15, maxQueueSize: 200, openingTime: "08:00", closingTime: "16:00", operatingDays: ["Mon", "Tue", "Wed", "Thu", "Fri"], todaysQueue: 55, completedToday: 50, avgWaitMins: 20, utilization: 75 },
  { id: "PASSPORT005", name: "Passport Renewal", department: "Immigration", location: "Lagos Office", status: "Inactive", queueEnabled: true, queuePrefix: "E", dailyCapacity: 100, avgServiceMins: 15, maxQueueSize: 200, openingTime: "08:00", closingTime: "16:00", operatingDays: ["Mon", "Tue", "Wed", "Thu", "Fri"], todaysQueue: 0, completedToday: 0, avgWaitMins: 0, utilization: 0 },
  { id: "PASSPORT006", name: "Passport Renewal", department: "Immigration", location: "Lagos Office", status: "Paused", queueEnabled: false, queuePrefix: "F", dailyCapacity: 100, avgServiceMins: 15, maxQueueSize: 200, openingTime: "08:00", closingTime: "16:00", operatingDays: ["Mon", "Tue", "Wed", "Thu", "Fri"], todaysQueue: 0, completedToday: 0, avgWaitMins: 0, utilization: 0 },
  { id: "PASSPORT007", name: "Passport Renewal", department: "Immigration", location: "Lagos Office", status: "Active", queueEnabled: true, queuePrefix: "G", dailyCapacity: 100, avgServiceMins: 15, maxQueueSize: 200, openingTime: "08:00", closingTime: "16:00", operatingDays: ["Mon", "Tue", "Wed", "Thu", "Fri"], todaysQueue: 33, completedToday: 30, avgWaitMins: 14, utilization: 55 },
];
...
const serviceApi = {
  async list() {
    // TODO: replace with `GET /service`
    return new Promise((resolve) => setTimeout(() => resolve(MOCK_SERVICES), 300));
  },
  async create(payload) {
    // TODO: replace with `POST /service`
    return new Promise((resolve) =>
      setTimeout(
        () =>
          resolve({
            ...payload,
            id: payload.serviceCode || `SVC${Date.now()}`,
            status: "Active",
            todaysQueue: 0,
            completedToday: 0,
            avgWaitMins: 0,
  ...
  async update(id, payload) {
    // TODO: replace with `PATCH /service/{id}`
    return new Promise((resolve) => setTimeout(() => resolve({ id, ...payload }), 300));
  },
  async deactivate(id) {
    // TODO: replace with `DELETE /service/{id}` or status PATCH
    return new Promise((resolve) => setTimeout(() => resolve({ id, status: "Inactive" }), 300));
  },
};
...
export default function ManagerServiceDashboard() {
  ...
  if (mode === "list" && services.length === 0) {
    return (
      <div className="svc-root">
        ...
        <div className="svc-empty">
          ...
          <p className="svc-empty-text">
            You have not create any service yet.
            <br />
    ...
    return (
      <div className="svc-root">
        ...
        <div className="svc-stats-row">
          ...
          <StatCard value={stats.paused} label="Pause Queue" tone="amber" />
        </div>
          ...
          <table className="svc-table">
            ...
            <tbody>
              {pagedServices.length === 0 ? (
                <tr>
                  <td colSpan={6} className="svc-table-empty">
                ...
                pagedServices.map((svc) => (
                  <tr key={svc.id}>
                    ...
                    <td>
                      <span
                        className={`svc-queue-pill ${svc.queueEnabled ? "is-enabled" : "is-disabled"}`}
                      >
          ...
          <div className="svc-pagination-controls">
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
              <button
                key={p}
                type="button"
                className={`svc-page-btn ${p === page ? "is-active" : ""}`}
                onClick={() => setPage(p)}
              >
                {p}
              </button>
    ...
    return (
      <div className="svc-root">
        <ServiceHeader
          ...
          subtitle={
            isEdit ? "Update the service information" : "Add a new service that customer can join"
          }
        />
        ...
        >
          <div className="svc-form-grid">
            ...
            <FormSection number={2} title="Location & Department">
            ...
            <FormSection number={3} title="Queue Configuration">
              ...
              <Field label="Daily Capacity" error={formErrors.dailyCapacity} required>
                <input
                  ...
                  placeholder="100"
                />
                <span className="svc-field-hint">Maximum customer per day</span>
              </Field>
              <Field label="Average Service Time" error={formErrors.avgServiceMins} required>
                <input
                  type="number"
                  min="1"
              ...
              <Field label="Maximum Queue Size" error={formErrors.maxQueueSize} required>
                <input
                  ...
                  placeholder="200"
                />
                <span className="svc-field-hint">Maximum people allowed in a queue</span>
              </Field>
            </FormSection>

            <FormSection number={3} title="Operation Hours">
        ...
        <div className="svc-detail-grid">
          <div className="svc-info-card">
            ...
            <dl className="svc-info-list">
              ...
              <InfoRow label="Average Service Time" value={`${svc.avgServiceMins} mins`} />
              <InfoRow label="Maximum Queue Size" value={svc.maxQueueSize} />
              <InfoRow
                label="Operating Hours"
                value={`${formatTime(svc.openingTime)} - ${formatTime(svc.closingTime)}`}
              />
          ...
          <div className="svc-detail-side">
            <div className="svc-today-card">
              ...
              <div className="svc-today-grid">
                <MiniStat value={svc.todaysQueue} label="Today's Queue" tone="neutral" />
                <MiniStat value={svc.completedToday} label="Completed Today" tone="teal" />
                <MiniStat value={`${svc.avgWaitMins} mins`} label="Average Wait Time" tone="amber" />
                <MiniStat value={`${svc.utilization}%`} label="Utilization" tone="blue" />
              </div>
            </div>
  ...
  // Fallback (e.g. activeService missing) — return to list
  backToList();
  return null;
}
...
function StatusBadge({ status }) {
  const cls =
    status === "Active" ? "is-active" : status === "Inactive" ? "is-inactive" : "is-paused";
...
function DeactivateModal({ onCancel, onConfirm, submitting }) {
  return (
    <div className="svc-modal-overlay" role="dialog" aria-modal="true" aria-labelledby="deactivate-title">
      <div className="svc-modal">
        <div className="svc-modal-icon">!</div>
        <h3 id="deactivate-title" className="svc-modal-title">
          Deactivate Service?
        </h3>
...
function WeekSparkline() {
  // Static decorative sparkline matching the Figma "Queue Summary" card.
  return (
    <svg viewBox="0 0 280 80" className="svc-sparkline" preserveAspectRatio="none" aria-hidden="true">
      <polyline
        points="0,55 40,40 80,48 120,25 160,35 200,15 240,30 280,18"
        fill="none"
        stroke="var(--svc-blue)"
        strokeWidth="2"
      />
      <polyline
        points="0,65 40,60 80,62 120,45 160,50 200,38 240,42 280,33"
        fill="none"
        stroke="var(--svc-teal)"
        strokeWidth="2"
      />
...
function FolderIllustration() {
  return (
    <svg width="180" height="140" viewBox="0 0 180 140" fill="none">
      ...
      <path
        d="M20 55 H80 L92 70 H160 A8 8 0 0 1 168 78 V112 A8 8 0 0 1 160 120 H20 A8 8 0 0 1 12 112 V63 A8 8 0 0 1 20 55 Z"
        fill="var(--svc-teal)"
      />
      <path
        d="M20 55 H80 L92 70 H160 A8 8 0 0 1 168 78 V82 H12 V63 A8 8 0 0 1 20 55 Z"
        fill="var(--svc-teal-dark)"
      />
...
// ---------------------------------------------------------------------
// Styles (CSS variables for theming, consistent with project conventions)
// ---------------------------------------------------------------------
...
.svc-field textarea:focus {
  outline: 2px solid var(--svc-teal);
  outline-offset: 1px;
...
.svc-today-card, .svc-summary-card {
  background: #fff;
  border-radius: 14px;
  border: 1px solid var(--svc-border);
  padding: 22px;
