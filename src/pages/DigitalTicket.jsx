export default function DigitalTicket({ ticket, position, orgName, service, onBackToQueue, onLeaveQueue }) {
  // Derive ticketState directly from position - no useEffect needed
  const ticketState = position === 0 ? "serving" : position === 1 ? "next" : "waiting";
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          {ticketState === "waiting" ? "IN QUEUE" : ticketState === "next" ? "NEXT UP" : "BEING SERVED"}
        </div>

        {/* Service Info in Card */}
      ...
      {ticketState === "next" && (
        <div style={{
          ...
          <div style={{ fontSize: 24, fontWeight: 700, color: "var(--teal)", marginBottom: 8 }}>
            You're Next!
          </div>
      ...
      {ticketState === "serving" && (
        <div style={{
          ...
          <div style={{ fontSize: 13, color: "var(--muted)" }}>
            Please report to counter immediately
          </div>
        </div>
      )}
